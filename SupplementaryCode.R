...
### Data loading
load("C:\\Users\\Justin\\Documents\\Data\\Food web modelling M-N Gravel et al 2016\\data.rdata")
str(data)
Plot.matrix(data[[1]])
...
Bprey <- data$BodySize_cal
Bpred <- data$Body_Size_med[1:length(Bprey)]
...
pairs <- NULL
	...
	for(i in 1:S){#i = rows = prey
		for(j in 1:S){ #j = columns = pred
